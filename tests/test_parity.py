...
CASES = [
    "", " ", "\n", "hello world", "  \n\n\t mixed \r\n whitespace  ",
    "it's I'M they'RE won't we'll you've", "'s 'S 'll 'LL 've 'd 'm 'x '",
    "def f(x):\n    return {'a': [1, 2]}  # comment\n",
    "0 1 12 123 1234 3.14159 1e-9 1_000_000",
    "ÀÉÎÕÜ àéîõü ĄĆĘŁŃÓŚŹŻ", "Привет, мир!", "العربية نص", "日本語のテキストです。",
    "中文测试，标点。", "한국어 테스트", "亚洲AV 中文AB混合Test",
    "🚀🎉😀 👨‍👩‍👧‍👦 🇺🇸🇯🇵", "ＡＢＣ ｆｕｌｌｗｉｄｔｈ", "MiXeD CamelCase SCREAMING_SNAKE",
    "https://example.com/a/b?c=d#e", "a" * 500, "日本" * 200, "word " * 200,
]
...
def test_random_unicode_fuzz(pair):
    ...
    for _ in range(1500):
        n = rng.randint(0, 40)
        s = "".join(
            chr(rng.choice([rng.randint(1, 0x2FFF), rng.randint(0x3000, 0xD7FF),
...
CORPORA = sorted(glob.glob(os.path.join(os.path.dirname(__file__), "..", "bench", "corpus", "*.txt")))
